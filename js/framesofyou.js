...
jQuery(function($) {
  ...
  var caller = $.ajax({ url: "http://api.followgram.com/v1/album/44485/1286686800/"
      }).success(function(data) {
      //console.log(data);
      parentHolder.addClass('processed').empty();
      $.each(data,function(i,v){
          if(i < limitIG) $('<img/>',{src:v.urlbig,"class":"ig_imgs"}).appendTo(parentHolder);
      });
      ...
      videoinfo: function(data) {
        ...
        for(var video in data) {
          var videoinfo = {
            ...
            thumbnails: {
              ...
              large: scale_thumbnail(
                ...
                'w=430&h=250&bg=000000&fit=crop&crop=faces'
                + '&mark=http://static.videofy.me/play-symbol-big.png'// (below)
                + '&markalign=middle,center'// Add play symbol
              ),
          ...
          var video_node = videos.filter(function() {
            return (
              $(this).data('video-id') == videoinfo.video_id
              && !$(this).is('.videofyme-api-populated')
            );
          });
...
jQuery(function($) {
  ...
  function create_dialog() {
    ...
    dialog.on('openmodaldialog', function() {
      background_fader.show(0).animate({"opacity": 0.5}, 'slow');
      dialog.slideDown('slow');
    });
    dialog.on('closemodaldialog', function() {
      background_fader.animate({"opacity": 0}, 'fast').hide(0);
...
jQuery(function($) {
  ...
  function maybe_activate() {
    ...
    more_link.addClass('activated').animate({opacity: 0});
      ...
      new_videos.on('videofymeapipopulated', function() {
        ...
        new_videos.each(function() {
          ...
          callbacks.push(function(callback) {
            ...
            video.css('opacity', 0);
            video.show(0);
            video.animate({ opacity: 1 }, 250, callback);
          });
        });
